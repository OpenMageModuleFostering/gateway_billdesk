...
/**
 *
 * BillDesk report handling class (URLOnline). Changes order state.
 * @author xxx
 */
class Gateway_Billdesk_Model_Report extends Mage_Core_Model_Abstract
{
  /**
   *
   * Request received from Billdesk
   * @var mixed
   */
  protected $request = null;
  /**
   *
   * Billdesk shop id received in the request
   * @var int
   */
  ...
  /**
   *
   * Response from the Billdesk payment information request
   * @var Gateway_Billdesk_Model_Transaction
   */
  ...
  public static $MSG_INVALID_SUM = 'Order total amount does not match Billdesk gross total amount.';
  ...
  public static $BILLDESK_CODE_TO_MAGENTO_STATUS = array(
  ...
  public function __construct() {
    ...
    $this->transaction = Mage::getModel('mbilldesk/transaction');
  }
  /**
   *
   * Extract params from the Billdesk request (save them in the model)
  ...
  /**
   *
   * Cancels order matching order_id = session_id from Billdesk
  ...
  /**
   *
   * Returns order object from session_id param from Billdesk, which is
  ...
  /**
   *
   * Save order to the database, and set the QuoteId from Billdesk. Unvalidate the quote
   */
  protected function saveOrder() {
    ...
    $session->setQuoteId($session->getBilldeskStandardQuoteId(true));
    Mage::getSingleton('checkout/session')->getQuote()->setIsActive(false)->save();
  }
  /**
   *
   * Check if current Billdesk transaction is a new order
   * @return true|false
   */
  protected function isNewOrder() {
    $ppStatus = @$this->transaction->getTransactionInfo()->status;
    if (self::$BILLDESK_CODE_TO_MAGENTO_STATUS[(int) $ppStatus]
  ...
  /**
   *
   * Check if current Billdesk transaction is a currently processed order
   * @return true|false
   */
  ...
  /**
   *
   * Check if current Billdesk transaction is a canceled order
   * @return true|false
   */
  ...
  /**
   *
   * After getting correct response from Billdesk with
   * transaction status.
   */
  protected function updateOrderState() {
    $validator = Mage::getModel('mbilldesk/transactionValidator');
  ...
  /**
   *
   * Entry point processing request form Billdesk
   * @param mixed $request from Billdesk
   */
  public function processRequest($request) {
    ...
    if ($this->checkParams() && $this->checkSignature()) {
      ...
      //Get transaction info from Billdesk
